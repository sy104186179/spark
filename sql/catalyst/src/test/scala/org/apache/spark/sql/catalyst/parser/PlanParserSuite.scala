...
class PlanParserSuite extends AnalysisTest {
  ...
  test("insert into") {
    ...
    def insert(
      ...
      InsertIntoTable(table("s"), Nil, partition, plan, overwrite, ifPartitionNotExists)
    ...
    assertEqual("from t insert into s select * limit 1 insert into u select * where x > 5",
      InsertIntoTable(
        table("s"), Nil, Map.empty, plan.limit(1), false, ifPartitionNotExists = false).union(
        InsertIntoTable(
          table("u"), Nil, Map.empty, plan2, false, ifPartitionNotExists = false)))
  }
  ...
  test("select hint syntax") {
    ...
    comparePlans(
      ...
      InsertIntoTable(table("s"), Nil, Map.empty,
        UnresolvedHint("REPARTITION", Seq(Literal(100)),
